// API para ocultar/publicar artigos do blog
// Adiciona atributo 'visible' ao artigo
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (fs.existsSync(HTML_PATH)) {
      ...
      // Padrão: "  slug:{...},visible:true" ou sem visible
      const regex = new RegExp(`(  ${slug}:\\{[^}]*(?:\\{[^}]*\\}[^}]*)*\\})(?:,visible:[a-z]+)?`, 'g');
    ...
    res.status(200).json({
      success: true,
      message: `Artigo ${visible ? 'publicado' : 'ocultado'} com sucesso!`,
      slug: slug,
      visible: visible
    });

  } catch (error) {
    console.error('Erro em toggle-article:', error);
    res.status(400).json({ error: error.message || 'Erro ao atualizar visibilidade do artigo' });
  }
};
